...
<template>
    <Transition enter-active-class="duration-200 ease-out" enter-from-class="transform opacity-0"
        ...
        <Modal @show="emit('update:show', false)" v-if="props.show" closable="true">
            <template #title>
                <h2 class="mb-2 text-xl font-bold"> Switch </h2>
            </template>
            <template #content>

                <Alert :show="!branches.length" type="error"
                    :text="`You need to create another <strong>branch</strong> or <strong>tag</strong>.`"
                    class="mb-4" />

                <template v-if="branches">
                    <Select vlabel="From:" :items="branches.map((b: any) => { return { name: b } })"
                        v-model:selected="selected" />
                </template>
            </template>
            <template #footer>
                <Button text="Switch" @click="onSwitch" class="mr-2 disabled:opacity-75" :disabled="!branches.length" />
